...
export const api = {
  ...
  // Visor AI
  visorChat: (message: string, provider?: string, model?: string, history?: { role: string; content: string }[]) =>
    request('/visor/chat', { method: 'POST', body: JSON.stringify({ message, provider: provider ?? 'ollama', model, history }) }),
